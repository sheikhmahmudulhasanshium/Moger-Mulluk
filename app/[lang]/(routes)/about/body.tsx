...
export default function Body() {
  ...
  // Helper for dynamic local text based on franchise regions
  const getRegionText = () => {
    switch(locale) {
  ...
  return (
    <div className="flex flex-col w-full bg-[#FFFBF8] dark:bg-stone-950 pb-20 transition-colors duration-300">
      ...
      <section className="relative z-20 -mt-10 md:-mt-16 text-center px-6">
        <div className="bg-white dark:bg-stone-900 inline-block px-8 py-6 md:px-12 md:py-10 shadow-xl dark:shadow-2xl dark:shadow-black/50 rounded-2xl border border-orange-50 dark:border-stone-800">
          <p className="text-[#B28869] font-bold tracking-[0.3em] uppercase text-[10px] mb-2">
            {getRegionText()}
          </p>
          <h1 className={`${getFontClass('black')} text-4xl md:text-6xl text-[#8A3D04] dark:text-orange-200 mb-2 tracking-tighter`}>
            {tAbout('title')}
          </h1>
          <p className={`${getFontClass()} text-lg md:text-xl text-[#B28869] dark:text-stone-400 italic font-medium`}>
            {tFooter('tagline')}
          </p>
        </div>
      </section>

      {/* 3. MULTINATIONAL IDENTITY SECTION */}
        ...
        <div className="relative aspect-video w-full bg-stone-900 rounded-4xl overflow-hidden shadow-2xl flex items-center justify-center group cursor-pointer border-8 border-white dark:border-stone-800">
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <p className="mt-6 text-white font-bold tracking-[0.2em] uppercase text-[10px] opacity-80">
              Our Journey: Local Roots, Global Wings
            </p>
          </div>
        </div>
      </section>

      {/* 5. VALUE PROPOSITIONS: GLOBAL STANDARDS */}
      ...
      <section className="pt-10 px-6 text-center">
        <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
          <Link 
            href="/menu" 
            className="w-full sm:w-auto px-12 py-5 bg-[#8A3D04] dark:bg-orange-700 text-white rounded-xl font-bold hover:shadow-2xl transition-all transform hover:-translate-y-1"
          >
            {tCTA('orderNow')}
          </Link>
          <Link 
            href="/faq" 
            className="w-full sm:w-auto px-12 py-5 border-2 border-[#8A3D04] dark:border-orange-800 text-[#8A3D04] dark:text-orange-200 rounded-xl font-bold hover:bg-[#8A3D04] dark:hover:bg-orange-800 hover:text-white transition-all"
          >
            {tFooter('quickLinks')}
          </Link>
        </div>
